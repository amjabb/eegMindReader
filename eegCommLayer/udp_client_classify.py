...
class UDPClient(object):
  ...
  def __init__(self, ip, port, model_file_name, rate, buffer_size=1024):
    ...
    self._firebase = firebase.FirebaseApplication("https://eegmindreader.firebaseio.com", None)
  ...
  def _read(self, condition=True):
          ...
          while condition:
                ...
                prediction = self._loaded_model.predict([sample])[0]
                #print prediction
                self.prediction_sum += prediction
                self.prediction_counter += 1
                if self.prediction_counter == self.sample_rate:
                  result = int(self.prediction_sum/self.prediction_counter)
                  self._firebase.put("/", "modelPrediction", str(result))
                  print(result)
                  self.prediction_counter = 0
                  self.prediction_sum = 0
...
UDPclient_sock = UDPClient(ip=args.host, port=int(args.port), rate=10, model_file_name=args.file)
